 ...
 */
class NamNode extends AudioWorkletNodeBase {
...
const enginesByContext = new WeakMap();
function resolveAssetUrl(assetBaseUrl, fallback, filename) {
...
class NamEngine {
     ...
     * the same engine).
     */
    static attach(context, assets) {
        let promise = enginesByContext.get(context);
        if (!promise) {
            promise = (async () => {
                if (!globalThis.AudioWorkletNode || !context.audioWorklet) {
                    throw new Error('AudioWorklet is not available. It requires a secure context: ' +
                ...
                // next to the assets.
                const workletUrl = resolveAssetUrl(assets?.assetBaseUrl, () => new URL('./nam-worklet.js', import.meta.url), 'nam-worklet.js');
                const wasmUrl = resolveAssetUrl(assets?.assetBaseUrl, () => new URL('./nam-engine.wasm', import.meta.url), 'nam-engine.wasm');
...
class NamNodePool {
     ...
     */
    constructor(engine, maxNodes = 1, onEvict) {
        this.engine = engine;
...
export { NamEngine, NamNode, NamNodePool };
